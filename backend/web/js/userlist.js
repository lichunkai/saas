layui.config({
	...
	//加载页面数据
	var usersData = '';
	$.get("index.php?r=site/getuserlist", function(data){
		usersData = [{
            "usersId" : "1",
            "userName" : "贤心",
	...
	$(".search_btn").click(function(){
		...
		if($(".search_input").val() != ''){
            ...
            setTimeout(function(){
            	$.ajax({
					url : "index.php?r=site/getuserlist",
					type : "get",
					dataType : "json",
	...
	$(".usersAdd_btn").click(function(){
		var index = layui.layer.open({
			title : "添加会员",
			type : 2,
			content : "index.php?r=site/adduser",
		...
		function renderDate(data,curr){
			...
			if(currData.length != 0){
				for(var i=0;i<currData.length;i++){
					...
					+    '<a class="layui-btn layui-btn-danger layui-btn-xs users_del" data-id="'+data[i].usersId+'"><i class="layui-icon">&#xe640;</i> 删除</a>'
			        +  '</td>'
			    	+'</tr>';
				}
			}else{
