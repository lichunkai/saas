 ...
 * 房屋出售订单控制器
 */
class OrderhighController extends AuthController
{
    ...
    public function actionGetsetting()
    {
        ...
        //加载自定义列表
        $customcolumns = CustomColumns::find()->select('columns')->where(['company_id'=>$company_id,'u_id'=>$this->_user['u_id'],'module'=>3,'is_del'=>0])->asArray()->one();
    ...
    public function actionGetindex()
    {
        ...
        $row = OrderSell::find()->where(['company_id'=>$company_id,'is_del' => 0,'order_type'=>3])->with(['created' => function ($query) {
...
//        $authwhere = Yii::$app->LoadData->checkDataByUser('orderhigh/getindex',$this->_user);
//        if($authwhere['key'] == 'none'){
//            return ApiReturn::success('无查看权限！');
//        }else if($authwhere['key'] != 'all'){
//            $columns = $authwhere['key'];
...
//        }
        ...
        foreach ( $data as $v )
        {
            ...
            $objectPHPExcel->getActiveSheet()->setCellValue('A1','高端成交');

            $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('A2','高端成交');
            $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('A2','高端成交');
