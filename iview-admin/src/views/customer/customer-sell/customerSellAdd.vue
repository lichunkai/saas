...
<template>
    <Modal v-model="addHouse" width="960" :closable="false" :mask-closable="false"
        ...
        <Form ref="formValidate" :model="formValidate"  :label-width="80">
			<Row>
				<i-col span="24" class='caddIcol f14' >客源基本信息</i-col>				
			</Row>
            <Row :gutter="5">
                <Col :lg="8" :md="8">
                <FormItem label="客户姓名" prop="customer_name" v-if="btx('customer_name')" :rules="{required: true, message: '姓名不能为空', trigger: 'blur'}" >
                    <Row>
                        <Col :lg="14" :md="14">
                        <Input v-model="formValidate.customer_name" placeholder=""></Input>
                        </Col>
                        <Col :lg="9" :md="9" style="margin-left: 5px;">
                ...
                <Col :lg="8" :md="8">
                <FormItem label="客户电话" prop="tel_phone" :rules="{required: true,pattern:/^(13[0-9]|14[5-9]|15[012356789]|166|17[0-8]|18[0-9]|19[8-9])[0-9]{8}$/, message: '请输入正确的电话号码', trigger: 'blur'}"
                          v-if="btx('tel_phone')">
                    <Row>
                        <Col :lg="14" :md="14">
                        <Input v-model="formValidate.tel_phone" placeholder=""></Input>
                        </Col>
                        <Col :lg="9" :md="9" style="margin-left: 5px;">
                        <Select placeholder="" :transfer="true" v-model="formValidate.tel_type">
                            <Option v-for="item in JSON.parse( this.peizhi.dianhuasuoshu[0].base_desp)"
                                    :value="item.child_name" :key="item.child_name">{{ item.child_name}}
                            </Option>
                        </Select>
                        </Col>
                ...
                <FormItem label="客户电话" prop="tel_phone"
                          v-if="!btx('tel_phone')">
                    <Row>
                        <Col :lg="14" :md="14">
                        <Input v-model="formValidate.tel_phone" placeholder=""></Input>
				...
				<Col :lg="8" :md="8">
					 <FormItem label="客户等级" prop="kehudengji"  v-if="btx('kehudengji')"  :rules="{required: true, message: '客户等级不能为空', trigger: 'change'}">
            ...
            <Row>
            	<i-col span="24" class='caddIcol f14'>客源主要信息</i-col>				
            </Row>
            <Row>
                <Col :lg="8" :md="8">
                <FormItem label="区域选择" prop="xuqiuquyu" v-if="btx('xuqiuquyu')"  :rules="{type: 'array',required: true, message: '需求区域不能为空', trigger: 'change'}">
                    <Cascader :data="this.peizhi.villages" v-model="formValidate.xuqiuquyu"
                              @on-change="changeVillage" filterable  change-on-select ></Cascader>
                </FormItem>
                ...
                <Col :lg="8" :md="8">
                <FormItem label="房屋用途" prop="yongtu"  v-if="btx('yongtu')"  :rules="{required: true, message: '房屋用途不能为空', trigger: 'change'}">
            ...
            <Row>
                <Col :lg="8" :md="8">
                <FormItem label="需求户型" prop="xuqiuhuxing" v-if="btx('xuqiuhuxing')"  :rules="{required: true, message: '需求户型不能为空', trigger: 'change'}">
                ...
                <Col :lg="8" :md="8">
                <FormItem label="需求价格" prop="xuqiujiage" v-if="btx('xuqiujiage')"  :rules="{required: true, message: '需求价格不能为空', trigger: 'change'}">
                ...
                <Col :lg="8" :md="8">
                <FormItem label="需求面积" prop="xuqiumianji" v-if="btx('xuqiumianji')"  :rules="{required: true, message: '需求面积不能为空', trigger: 'change'}">
            ...
            <Row>
                <Col :lg="8" :md="8">
                <FormItem label="需求楼层" prop="xuqiulouceng" v-if="btx('xuqiulouceng')"  :rules="{required: true, message: '需求楼层不能为空', trigger: 'change'}">
				...
				<Col :lg="8" :md="8">
				<FormItem label="需求房龄" prop="xuqiufangling" v-if="btx('xuqiufangling')"  :rules="{required: true, message: '需求房龄不能为空', trigger: 'change'}">
                ...
                <Col :lg="8" :md="8">
                <FormItem label="房屋朝向" prop="chaoxiang" v-if="btx('chaoxiang')"  :rules="{required: true, message: '朝向不能为空', trigger: 'change'}">
                ...
                <Col :lg="8" :md="8">
                <FormItem label="装修需求" prop="zhuangxiu" v-if="btx('zhuangxiu')"  :rules="{required: true, message: '装修不能为空', trigger: 'change'}">
				...
				<Col :lg="16" :md="16">
				<FormItem label="内部配套" prop="peitao" v-if="btx('peitao')"  :rules="{type:'array',required: true, message: '配套不能为空', trigger: 'change'}">
            ...
            <Row>
				<Col :lg="8" :md="8">
                 <FormItem label="房屋类型" prop="fangwuleixing" v-if="btx('fangwuleixing')"  :rules="{required: true, message: '推荐标签不能为空', trigger: 'change'}">
				...
				<Col :lg="16" :md="16">
				<FormItem label="特殊需求" prop="tiaojianbiaoqian" v-if="btx('tuijianbianqian')"  :rules="{type: 'array',required: true, message: '特殊需求不能为空', trigger: 'change'}">
                ...
                <Col :lg="24" :md="24">
                <FormItem label="需求原因" prop="xuqiuyuanying" v-if="btx('xuqiuyuanying')"  :rules="{required: true, message: '需求原因不能为空', trigger: 'blur'}">
                    <!-- <Input v-model="formValidate.xuqiuyuanying"></Input> -->
					 <i-input type="textarea" placeholder="请输入..." v-model="formValidate.xuqiuyuanying"></i-input>
                </FormItem>
            ...
            <Row>
                <Col :lg="24" :md="24">
                <FormItem label="标签选择" prop="duoxuanbiaoqian" v-if="btx('duoxuanbiaoqian')"  :rules="{type: 'array',required: true, message: '多选标签不能为空', trigger: 'change'}">
			...
			<Row>
				<i-col span="24" class='caddIcol f14'>客源次要信息</i-col>				
			</Row>
            <Row>
                <Col :lg="8" :md="8">
                <FormItem label="沟通阶段" prop="goutongjieduan" v-if="btx('goutongjieduan')"  :rules="{required: true, message: '沟通阶段不能为空', trigger: 'change'}">
                ...
                <Col :lg="8" :md="8">
                <FormItem label="客户来源" prop="kehulaiyuan" v-if="btx('kehulaiyuan')"  :rules="{required: true, message: '客户来源不能为空', trigger: 'change'}">
                ...
                <!-- <Col :lg="6" :md="6">
                <FormItem label="国籍" prop="guoji" v-if="btx('guoji')"  :rules="{required: true, message: '国籍不能为空', trigger: 'blur'}">
            ...
            <Row>
              <!--  <Col :lg="6" :md="6">
                <FormItem label="民族" prop="minzu" v-if="btx('minzu')"  :rules="{required: true, message: '民族不能为空', trigger: 'blur'}">
                ...
                <Col :lg="8" :md="8">
                <FormItem label="证件号码" prop="zhengjianhaoma" v-if="btx('zhengjianhaoma')"  :rules="{required: true, message: '证件号码不能为空', trigger: 'blur'}">
                ...
                <Col :lg="8" :md="8">
                <FormItem label="客户邮箱" prop="email"  v-if="btx('email')"  :rules="{required: true,pattern:/^[a-z0-9]+([._\\-]*[a-z0-9])*@([a-z0-9]+[-a-z0-9]*[a-z0-9]+.){1,63}[a-z0-9]+$/, message: '请输入正确的邮箱', trigger: 'blur'}">
                ...
                <FormItem label="客户邮箱" prop="email" v-if="!btx('email')">
				 ...
				 <Col :lg="8" :md="8">
				 <FormItem label="客户微信" prop="weixin"  v-if="btx('weixin')"  :rules="{required: true, message: '微信不能为空', trigger: 'blur'}">
               ...
               <!-- <Col :lg="8" :md="8">
                <FormItem label="客户QQ" prop="qq" v-if="btx('qq')"  :rules="{required: true, message: 'qq不能为空', trigger: 'blur'}">
                    <Input v-model="formValidate.qq"></Input>
                </FormItem>
                <FormItem label="客户QQ" prop="qq" v-if="!btx('qq')">
                    <Input v-model="formValidate.qq" ></Input>
                </FormItem>
                </Col> -->
            </Row>
            <Row>
               ...
               <!-- <Col :lg="6" :md="6">
                <FormItem label="交通工具" prop="jiaotonggongju" v-if="btx('jiaotonggongju')"  :rules="{required: true, message: '交通工具不能为空', trigger: 'blur'}">
                ...
                <Col :lg="8" :md="8">
                <FormItem label="客户车型" prop="chexing" v-if="btx('chexing')"  :rules="{required: true, message: '车型不能为空', trigger: 'blur'}">
            ...
            <Row>
                <Col :lg="24" :md="24">
                <FormItem label="备注" prop="mark" v-if="btx('mark')"  :rules="{required: true, message: '备注不能为空', trigger: 'blur'}">
                ...
                <!-- <Col :lg="12" :md="12">
                <FormItem label="核心备注" prop="core_mark" v-if="btx('core_mark')"  :rules="{required: true, message: '备注不能为空', trigger: 'blur'}">
...
<script>
    export default {
        ...
        props: ['peizhi', 'addHouse', 'formValidate'],
        data () {
            return {
                isDisable:false,
                village:'',
        ...
        methods: {
            ...
            changeVillage (value, selectedData) {//设置小区信息
            ...
            modalOk () {
                ...
                this.$refs['formValidate'].validate((valid) => {
                    if (valid) {
                        this.isDisable=true;
                        this.$http.post(api_param.apiurl + '/customer/add',
                            data,
