    ...
    <Row class="editHight">
        ...
        <Card>
            <Row>
                ...
                <editDaikan :rDaikan="rDaikan" :buyDealtitle="buyDealtitle" :peizhi="peizhi"
                            v-on:resetok1="resetok1" v-on:get_daikanlist="get_daikanlist"></editDaikan>
                </Col>
                <Col class="roomCol">
        ...
        <Card>
            <Tabs type="card" :animated="false" v-model="tabskey.key">
                <TabPane label="基本信息">
                    <editBasic :customer="customer" :peizhi="peizhi" :genjinlist="genjinlist" :daikanlist="daikanlist" :teltrue="teltrue" v-on:follow_tel="follow_tel" v-on:resetok="resetok"></editBasic>
                </TabPane>
                <TabPane label="匹配房源">
                    <editCustomers ref="fangyuanpipei" :customer="customer" :peizhi="peizhi" v-on:resetok="resetok"></editCustomers>
                </TabPane>

               <!-- <TabPane label="相似客户">
                    <editSimilar ref="xiangshikehu" :customer="customer" :peizhi="peizhi"   v-on:resetok="resetok"></editSimilar>
                </TabPane> -->
                <TabPane label="客源跟进" >
                    <editGenjin ref="mygenjin" :genjinlist="genjinlist"></editGenjin>
                </TabPane>
                <TabPane label="带看记录">
                    <editSee ref="mydaikan" :daikanlist="daikanlist"></editSee>
                </TabPane>
                <TabPane label="客源日志">
...
<script>
    ...
    import editDaikan from './editDaikan.vue';
    ...
    export default {
        ...
        methods: {
            ...
            resetok1() {
				...
				this.formValidate.kehudengji = this.customer.kehudengji;
			...
			//跟进列表
			get_genjinlist() {
			    this.$http.get(api_param.apiurl + '/customer_follow/index',
			        {
			            params: {
			        ...
			        // 这里是处理正确的回调
			        if (response.data.code == 200) {
			            this.genjinlist = response.body.data.list;						
			...
			get_daikanlist() {
			    this.$http.get(api_param.apiurl + '/customer_daikan/index',
			        ...
			        // 这里是处理正确的回调
			        if (response.data.code == 200) {
			            this.daikanlist = response.body.data.list;
			        } else if (response.data.code == 401) {
			            this.$store.commit('logout', this);
			            this.$store.commit('clearOpenedSubmenu');
			            this.$router.push({
