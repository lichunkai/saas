...
<template>
    <Row>
        ...
        <Row>
            ...
            <Row :gutter="40">
                <Col :lg="8" :md="8" v-for="setting in settings.house_imgs">
                <div class="demo-upload-list" v-for="img in data.images[setting.type]">
                    <img :src="imgurl + img.hi_url">
                    <div class="demo-upload-list-cover">
                        <Icon type="ios-eye-outline" @click.native="handleView(img.hi_url)"></Icon>
                        <Icon v-if="data.images_show  == 1" type="ios-trash-outline" @click.native="handleRemove(img.hi_url)"></Icon>
                    </div>
                    <Progress v-if="showProgress[setting.type]" :percent="percentage[setting.type]" hide-info></Progress>
                </div>

                <Upload v-if="data.images[setting.type].length < 1"
                        ref="upload"
                ...
                <Col :lg="24" :md="24">
                    <br><br>
                    <Button v-if="data.images_show  == 1" type="primary" style="width: 200px" @click="saveImg">保存</Button>
                    <br><br>
                </Col>

                <Modal title="查看大图" v-model="visible" style="hight:560px">
                    <div slot="header">
                    ...
                    <Carousel v-if="visible" v-model="imgIndex" dots="none" value="0">
                        <CarouselItem v-for="imgs in data.imglist">
		...
		methods: {
			...
			download() {
				...
				for (let i in this.data.imglist) {
					if (this.data.imglist[i] !== false&& this.data.imglist[i].hi_type  && this.data.imglist[i].hi_id) {
						let settingIndex=0;
						if(this.data.imglist[i].hi_type<16){
							settingIndex=this.data.imglist[i].hi_type-1;
						}else{
							settingIndex=this.data.imglist[i].hi_type-6;
						}
						let urlarr = this.data.imglist[i].hi_url.split('.');
						downloadimages.push({'image_src': api_param.imgurl + this.data.imglist[i].hi_url, 'image_name': this.settings.house_imgs[settingIndex].name + this.data.imglist[i].hi_id + '.' + urlarr[1]});
					}
				}
			...
			handleView(url) { //查看大图
				for (let i in this.data.imglist) {
					if (this.data.imglist[i].hi_url == url) {
			...
			handleRemove(url) { //删除事件
				for (let i in this.data.imglist) {
					if (this.data.imglist[i].hi_url == url) {
                        //删除images
                        for(let img in this.data.images[this.data.imglist[i].hi_type]){
                        	if(this.data.images[this.data.imglist[i].hi_type][img].hi_url ==url){
		                        this.data.images[this.data.imglist[i].hi_type].splice(img,1);
                            }
                        }
						//删除imglist
			...
			handleSuccess(res, file, filelist) {//上传成功
				...
				this.$set(this.data.images[type], this.data.images[type].length, tmp);
				this.$set(this.data.imglist, this.data.imglist.length, tmp);
			},
			...
			saveImg() {
				...
				for (let i in this.data.images) {
					if (i <= 11||i>16) {
						if(this.data.images[i].length>0){
							images[i] = this.data.images[i];
                        }else{
                            this.$Message.warning('请上传全部图片后保存');
                            return false
                        }
					}
				}
